...
<section class="flex items-center justify-center min-h-screen bg-gray-50">
    <!-- login container -->
    <div class="flex items-center max-w-3xl p-5 bg-purple-100 shadow-lg rounded-2xl">
        <!-- image -->
        ...
        <div class="px-8 md:w-1/2 md:px-16">
            ...
            <form action="{{ route('register') }}" method="post" class="flex flex-col gap-4 ">
                @csrf
                <input class="p-2 mt-8 border rounded-xl" type="text" name="name" placeholder="Name"
                    value="{{ old('name') }}" autocomplete="off">
                @error('name')
                <span class="text-sm text-red-500">{{ $message }}</span>
                @enderror

                <input class="p-2 border rounded-xl" type="email" name="email" placeholder="Email"
                ...
                <div class="relative" x-data="{ showPassword: false}">
                    ...
                    <svg @click="showPassword = !showPassword" xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                        fill="gray" class="absolute -translate-y-1/2 bi bi-eye top-1/2 right-3" viewBox="0 0 16 16">
                        <path
                ...
                <button type="submit"
                    class="bg-[#002D74] rounded-xl text-white py-2 hover:scale-105 duration-300 mt-5">Register</button>
            </form>
            ...
            <div class="mt-3 text-xs flex justify-between items-center text-[#002D74]">
                ...
                <a href="login"><button
                        class="px-5 py-2 duration-300 bg-white border rounded-xl hover:scale-110">Login</button></a>
