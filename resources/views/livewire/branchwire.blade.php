    ...
    <div class="flex w-2/3 p-3 mx-auto items-top justify-evenly">
        <div class="w-1/2 p-4 mt-3 mr-3 text-white bg-teal-500 border shadow-xl rounded-xl">
            ...
            <div class="w-full mt-3">
                <label class="px-2 block" for="kota">{{ __('Kota') }}</label>
                <select id="kota" wire:model="kota_id"
            ...
            <div class="w-full mt-3">
                ...
                <input id="kode_vihara" type="text" placeholder="{{ __('Kode Vihara') }}" wire:model="kode_branch"
                    class="w-full my-3 text-gray-700 rounded-lg shadow-sm focus:border-purple-500 focus:ring-purple-500">
                @error('kode_branch')
                    <span class="text-red-500">{{ $message }}</span>
                @enderror
                ...
                <table class="w-full table-auto">
                    <thead class="text-white bg-purple-500 border-b-2 border-gray-200 rounded-xl">
                        <tr>
                            ...
                            <th class="p-3 text-sm font-semibold text-center">{{ __('Nama Vihara') }}</th>
                            <th class="p-3 text-sm font-semibold text-center">{{ __('Kode Vihara') }}</th>
                            <th class="p-3 text-sm font-semibold text-center">{{ __('Action') }}</th>
                        </tr>
                    </thead>
                    @foreach ($branch as $index => $b)
                        <tbody>
                            <tr>
                                ...
                                <td class="p-3 text-sm text-gray-800 border rounded">{{ $b->nama_kota }}</td>
                                <td class="p-3 text-sm text-gray-800 border rounded">{{ $b->nama_branch }}</td>
                                <td class="p-3 text-sm text-gray-800 border rounded">{{ $b->kode_branch }}</td>
                                <td class="p-3 text-sm text-center text-gray-800 border rounded">
                                    ...
                                    @else
                                        <button class="button button-teal"
                                            wire:click="edit({{ $b->id }})">{{ __('Rename') }}</button>
                                    @endif
                                </td>
                            </tr>
                        </tbody>
